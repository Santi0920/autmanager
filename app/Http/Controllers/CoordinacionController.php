...
class CoordinacionController extends Controller
{
    ...
    public function solicitudes(Request $request)
    {
        ...
        if ($agenciaU = $usuarioActual->agenciau == "Coordinacion 1") {
            ...
            WHERE (B.Solicitud = 1 AND B.NumAgencia IN (34, 36, 37, 38, 40, 41, 87, 93, 96, 'C1')) OR (B.Estado = 6)");
            ...
            JOIN autorizaciones B ON B.ID_Persona = A.ID
            JOIN concepto_autorizaciones C ON B.ID_Concepto = C.ID
            WHERE (B.Solicitud = 1 AND B.NumAgencia IN (32, 42, 47, 81, 82, 83, 85, 90, 94, 'C3')) OR (B.Estado = 6)");
            ...
            JOIN autorizaciones B ON B.ID_Persona = A.ID
            JOIN concepto_autorizaciones C ON B.ID_Concepto = C.ID
            WHERE (B.Solicitud = 1 AND B.NumAgencia IN (44, 45, 48, 49, 74, 75, 84, 89, 91, 95, 97 'C4')) OR (B.Estado = 6)");
            ...
            JOIN autorizaciones B ON B.ID_Persona = A.ID
            JOIN concepto_autorizaciones C ON B.ID_Concepto = C.ID
            WHERE (B.Solicitud = 1 AND B.NumAgencia IN (13, 30, 31, 43, 68, 73, 76, 86 'C5')) OR (B.Estado = 6)");
        }
