...
<body class="antialiased">
        ...
        <div class="">
            <form action="" method="post">
                <div class="d-flex justify-content-between align-items-center" style="margin-top: 8px; margin-right: -14px;">
                    <span class="d-inline mb-0 text-dark text-end" style="font-size: 35px"><b><a class="text-dark" href="aprobar" title="REGRESAR"><i class="fas fa-arrow-left"></i></a>&nbsp;&nbsp;⭐FILTRAR POR CONCEPTO⭐</b></span>
            ...
            <table id="personas" class="hover table table-striped shadow-lg mt-4 table-hover table-bordered">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td>
                            <select name="" id="" data-column="3" class="form-control filter-select fs-4 fw-bold">
                                <option value="" class="fw-bold">AGENCIA<option>
                                @foreach ($agencia as $agencia)
                                        <option class="fw-semibold" value="{{ $agencia->NomAgencia }}">
                                            {{ $agencia->NomAgencia }}
                                        </option>
                                @endforeach
                            </select>
                        ...
                        <td>
                            <select name="" id="" data-column="8" class="form-control filter-select fs-4 fw-bold">
                                <option value="" class="fw-bold">SOLICITADO<option>
                                @foreach ($solicitadopor as $solicitadopor)
                                        <option class="fw-semibold" value="{{ $solicitadopor->SolicitadoPor }}">
                        ...
                        <td>
                            <select name="" id="monthYearSelect" data-column="10" class="form-control filter-select fs-4 fw-bold w-100">
                                <option value="" class="fw-bold">ESTADO</option>
                                {{-- <option value="VALIDADO" class="">VALIDADO</option> --}}
                                <option value="APROBADO" class="">APROBADO</option>
                                <option value="RECHAZADO POR COORDINACIÓN" class="">RECHAZADO POR COORDINACIÓN</option>
                                <option value="RECHAZADO POR GERENCIA" class="">RECHAZADO POR GERENCIA</option>
                                <option value="EN TRAMITE" class="">EN TRAMITE</option>
                                <option value="BLOQUEADO" class="">BLOQUEADO</option>
                            </select>
                        </td>
                        <td>
                            <select name="" id="" data-column="11" class="form-control filter-select fs-4 fw-bold">
                                <option value="" class="fw-bold">VALIDADO<option>
                                @foreach ($validadopor as $validadopor)
                                        <option class="fw-semibold" value="{{ $validadopor->ValidadoPor }}">
    ...
    <script>
        var table = $('#personas').DataTable({
            "ajax": "{{ route('datager.filtrarconcepto') }}",
            "processing" : true,
            "order": [
                [0, 'desc']
            ],
...

                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (row.Estado == 1) {
                            ...
                            var Estado =
                                '<div class="btn btn-danger shadow" style="padding: 0.4rem 1.6rem; border-radius: 10%;font-weight: 600;font-size: 14px;"><label style="margin-bottom: 0px;">RECHAZADO POR COORDINACIÓN</div>'
                        ...
                        }else if(row.Estado == 2 && row.Coordinacion == 'C#'){
                            var Estado =
                                '<div class="btn btn-warning shadow" style="padding: 0.4rem 1.6rem; border-radius: 10%;font-weight: 600;font-size: 14px;"><label style="margin-bottom: 0px;">EN TRAMITE</div>'
                        }else if(row.Bloqueado == 1){
                            var Estado =
                                '<div class="btn btn-info shadow" style="padding: 0.4rem 1.6rem; border-radius: 10%;font-weight: 600;font-size: 14px;"><label style="margin-bottom: 0px;">BLOQUEADO</div>'
                        }
            ...
            responsive: "true",
                    ...
                    buttons:[
                        {
                            extend:    'excelHtml5',
                            text:      '<i class="fas fa-file-excel">&nbsp&nbsp&nbspD E S C A R G A R &nbsp&nbsp E X C E L</i> ',
